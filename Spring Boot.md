...
>
> ​		通过注解的类拿到注解元数据，这个元数据获取要加载的包名【主配置类（@SpringBootApplication标注的类）的所在包及下面所有子包里面的所有组件扫描到Spring容器】
>
> ![](images/register.png)
>
> @**Import**(EnableAutoConfigurationImportSelector.class)
...
>
> ![](images\selectImport.png)	
>
> ​		SpringFactoriesLoader.loadFactoryNames(EnableAutoConfiguration.class,classLoader)
>
> ![](images/Candidate.png)
>
> ![](images/loadFactory.png)
>
> ​		Spring Boot在启动的时候从类路径下的META-INF/spring.factories中获取EnableAutoConfiguration指定的值，将这些值作为自动配置类导入到容器中，自动配置类就生效，帮我们进行自动配置工作；以前我们需要自己配置的东西，自动配置类都帮我们；
>
> ![](images/factoryLocation.png)
>
> ![自动配置类](images/搜狗截图20180129224104.png)	
>
> ​		有了自动配置类，免去了我们手动编写配置注入功能组件等的工作；
>
> ​		J2EE的整体整合解决方案和自动配置都在spring-boot-autoconfigure-xxx.jar；
>
...
​		可以直接在测试的时候，配置传入命令行参数（先properties，再yml）
...
![](images/ex-config2.png)

## 8、自动配置原理

配置文件到底能写什么？怎么写？自动配置原理；

[配置文件能配置的属性参照](https://docs.spring.io/spring-boot/docs/2.1.2.RELEASE/reference/htmlsingle/#common-application-properties)
...
（slf4j，log4j，logback都是同一个人写的，由于log4j有性能问题，所有用logback	）

（log4j2 由apache模仿log4j做的）

（jul 是在log4j出现后，官方加载jdk中的自带log）
...
​	3）中间替换包，虽然以apache为包名，但内部实现都是slf4j
...
​	4）、如果要引入其他框架，一定要把这个框架的默认日志依赖移除掉（避免发生包名冲突问题）

（Spring框架用的是commons-logging，但其实内部已经默认以slf4j代替实现）
...
localhost:8080/assert/favicon.js ===  去静态资源文件夹+assert/里面找favicon.js
...
<body>
	...
	<p th:utext="${h1}"></p>
	<hr>
	<h3 th:each="us:${user}" th:text="${us}"></h3>
	<h3 >
		<span th:each="us:${user}">[[${us}]]</span>
	</h3>
</body>
</html>
```
...
- Inclusion of `ContentNegotiatingViewResolver` and `BeanNameViewResolver` beans.
  ...
  ![](images/ContentNegotiating.png)

  ![](/assets/candidateview.png)

  ![](images/viewresolver.png)

  ​	自定义viewResolver解析

  ![](images/myresolver.png)

  ​	访问任意页面，任何访问都会通过该类的方法拦截，查看当前类是否有自定义的解析器

  ![](images/dispatchservlet.png)
...
```java
		...
		public FormattingConversionService mvcConversionService() {
			WebConversionService conversionService = new WebConversionService(
					this.mvcProperties.getDateFormat());//通过mvcProperties的配置获取format
...
- Support for `HttpMessageConverters` (see below).
  ...
  - HttpMessageConverter：SpringMVC用来转换Http请求和响应的；将user以json数据返回；
...
public class MyMvcConfig implements WebMvcConfigurer {
    ...
    public void addViewControllers(ViewControllerRegistry registry) {
       // super.addViewControllers(registry);
        //浏览器发送 /lov 请求来到 success
        registry.addViewController("/lov").setViewName("success");
    }
}
```
...
​	3）、由上分析：容器中所有的WebMvcConfigurer都会一起起作用；

​	4）、所以自定义的配置类也会被调用；

​	效果：SpringMVC的自动配置和自定义的扩展配置都会起作用；
...
2）、导入的DelegatingWebMvcConfiguration集成WebMvcConfigurationSupport 
...
3）、判断是否存在WebMvcConfigurationSupport 类
...
4）、由于2），所以WebMvcAutoConfiguration不加载配置
...
2）、页面修改完成以后重新编译`ctrl+f9`（IDEA）`ctrl+shift+f9`（eclipse）
...
先在标有@configuration的任意加载配置类中加入@ServletComponentScan注解
...
WebServerFactoryCustomizerBeanPostProcessor：定制器帮我们修改了Servlet容器的配置？
...
private Collection<WebServerFactoryCustomizer<?>> getCustomizers() {
		if (this.customizers == null) {
			...
			this.customizers = new ArrayList<>(getWebServerFactoryCustomizerBeans());//从容器中获取所有这类型的组件：WebServerFactoryCustomizer
...
​	从ioc容器中获取**WebServerFactory** 组件；**TomcatServletWebServerFactory**创建对象，后置处理器一看是这个对象，就获取所有的定制器来先定制Servlet容器的相关配置；
...
​		缺点：默认不支持JSP、优化定制比较复杂（使用定制器【ServerProperties、自定义WebServerFactoryCustomizer】，自己编写嵌入式Servlet容器的创建工厂【WebServerFactory】）；
...
spring:
  datasource:
    username: root
    password: ${password}
    url: jdbc:mysql:///lov?useSSL=false
    driver-class-name: com.mysql.jdbc.Driver
    schema: 
      - classpath:department.sql
